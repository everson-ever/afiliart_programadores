...
class ProgrammerController {
	...
	async show(req, res) {
		try {
			...
			if (err.name === 'CastError') {
				return res.status(404).json(new Response(404, 'recurso não encontrado', null));
			}
			return res.status(500).json(new Response(500, 'erro', null));
		}
	}
	...
	async update(req, res) {
		try {
			...
			const programmer = await ProgrammerSchema.findOneAndUpdate({ _id: id }, req.body, {
				new: true
			});
			...
			if (err.name === 'CastError') {
				return res.status(400).json(new Response(400, 'recurso não encontrado', null));
			}
			return res.status(500).json(new Response(500, 'erro', null));
		}
	}
